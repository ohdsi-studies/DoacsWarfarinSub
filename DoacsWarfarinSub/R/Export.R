...
exportAnalyses <- function(outputFolder, exportFolder, timeAtRisk) {
  ...
  cmAnalysisListFile <- system.file("settings",
                                    "cmAnalysisList.json",
                                    package = "DoacsWarfarinSub")
  cmAnalysisList <- CohortMethod::loadCmAnalysisList(cmAnalysisListFile)
  
  if (timeAtRisk == "OnTreatment") {
    cmAnalysisList <- cmAnalysisList[c(1,3)]
    # cmAnalysisList[1] "1:1 ratio PS matching on-treatment unconditional Cox PH"
    # cmAnalysisList[3] "1:100 variable ratio PS matching on-treatment conditional Cox PH"
  }
  if (timeAtRisk == "IntentToTreat") {
    cmAnalysisList <- cmAnalysisList[c(2,4)]
    # cmAnalysisList[2] "1:1 ratio PS matching intent-to-treat unconditional Cox PH"
    # cmAnalysisList[4] "1:100 variable ratio PS matching intent-to-treat conditional Cox PH"
  }
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "DoacsWarfarinSub")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("settings", "CohortsToCreate.csv", package = "DoacsWarfarinSub")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "DoacsWarfarinSub")
  ...
  preparePlot <- function(row, reference) {
    ...
    if (file.exists(psFileName)) {
      ...
      if (min(ps$propensityScore) < max(ps$propensityScore)) {
        ...
        if (length(ps$preferenceScore[ps$treatment == 1]) > 1) {
          d1 <- density(ps$preferenceScore[ps$treatment == 1], from = 0, to = 1, n = 100)
        } else {
          d1 <- density(rep(ps$preferenceScore[ps$treatment == 1], 2), from = 0, to = 1, n = 100)
        }
        if (length(ps$preferenceScore[ps$treatment == 0]) > 1) {
          d0 <- density(ps$preferenceScore[ps$treatment == 0], from = 0, to = 1, n = 100)
        } else {
          d0 <- density(rep(ps$preferenceScore[ps$treatment == 0], 2), from = 0, to = 1, n = 100)
        }

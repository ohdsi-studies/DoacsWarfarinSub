...
#'                             (/)
#' @param timeAtRisk           Time at risk label for calling correct positiveControlSynthArgs                             
...
synthesizePositiveControls <- function(connectionDetails,
  ...
  #synthesisFolder <- file.path(outputFolder, paste0("positiveControlSynthesis", timeAtRisk))
  if (!file.exists(synthesisFolder)) {
    dir.create(synthesisFolder)
  }
  
  synthesisSummaryFile <- file.path(outputFolder, "SynthesisSummary.csv")
  #synthesisSummaryFile <- file.path(outputFolder, paste0("SynthesisSummary", timeAtRisk, ".csv"))
  if (!file.exists(synthesisSummaryFile)) {
    ...
    pathToJson <- system.file("settings", paste0("positiveControlSynthArgs", timeAtRisk, ".json"), package = "DoacsWarfarinSub") 
  ...
  #write.csv(allControls, file.path(outputFolder, paste0("AllControls", timeAtRisk, ".csv")), row.names = FALSE)
}

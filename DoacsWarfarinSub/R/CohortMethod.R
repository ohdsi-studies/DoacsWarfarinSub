...
#'                             performance.
#' @param timeAtRisk           Time at risk label for calling correct cmAnalysisList elements                             
...
runCohortMethod <- function(connectionDetails,
                            ...
                            timeAtRisk,
                            maxCores) {
  #cmOutputFolder <- file.path(outputFolder, paste0("cmOutput", timeAtRisk))
  ...
  tcosList <- createTcos(outputFolder = outputFolder) #, timeAtRisk = timeAtRisk)
  ...
  #write.csv(analysisSummary, file.path(outputFolder, paste0("analysisSummary", timeAtRisk, ".csv")), row.names = FALSE)

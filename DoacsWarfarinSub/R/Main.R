...
#'
#' @details
#' This function executes the DoacsWarfarinSub Study.
#' 
...
execute <- function(connectionDetails,
  ...
  timeAtRisks <- c("OnTreatment", "IntentToTreat") # timeAtRisk <- "IntentToTreat"
  ...
  if (doPositiveControlSynthesis) {
    if (synthesizePositiveControls) {
      ...
      for (timeAtRisk in timeAtRisks) {
        tarOutputFolder <- file.path(outputFolder, timeAtRisk)
        if (!file.exists(tarOutputFolder)) {
          dir.create(tarOutputFolder, recursive = TRUE)
        }
  ...
  if (runDiagnostics) {
    ...
    for (timeAtRisk in timeAtRisks) {
      tarOutputFolder <- file.path(outputFolder, timeAtRisk)
      generateDiagnostics(outputFolder = tarOutputFolder,
                          maxCores = maxCores)
    }
  }
  ...
  if (packageResults) {
    ...
    for (timeAtRisk in timeAtRisks) {
      tarOutputFolder <- file.path(outputFolder, timeAtRisk)
      exportResults(outputFolder = tarOutputFolder,
    ...
    dummyFolder <- file.path(outputFolder, timeAtRisks[1], "export")
    ...
    for (resultName in resultNames) {
      ...
      for (timeAtRisk in timeAtRisks) {
        resultsFolder <- file.path(outputFolder, timeAtRisk, "export")

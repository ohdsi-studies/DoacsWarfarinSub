...
launchEvidenceExplorer <- function(dataFolder, blind = TRUE, launch.browser = TRUE) {
  ensure_installed("DT")
  appDir <- system.file("shiny", "EvidenceExplorer", package = "DoacsWarfarinSub")
...
compileShinyData <- function(studyFolder,
  ...
  for (database in databases) {
    folder <- file.path(studyFolder, database, "shinyData")
    fileNames <- list.files(folder, pattern = ".*\\.rds$")
    if (database == "MetaAnalysis") {
      database <- "Meta-analysis"
    }
    fileNames <- sub(paste0("_", database, ".rds"), "", fileNames)
    resultNames <- c(resultNames, fileNames)
  }

...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (197236)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (197236)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (2004444,2004449,2004503)
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (2101815,2101809,2101015,2101000,2101808,2101597,2101592,2004524,2110171,42733496,2110163,4015660,4141940,2110203,42739908,2110204,1314496,1314497,4127886,2110228,2722224,2722227,2110223,2722223,2110226,2722226,2722225,2722222,2110292,2722207,2868153,2802049,2004527,2004491,195599,2722229,2721144,40756770,40756808,2108349)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (2101815,2101809,2101015,2101000,2101808,2101597,2101592,2110171,42733496,2110163,2110203,42739908,2110204,1314496,1314497,4127886,2110228,2722224,2722227,2110223,2722223,2110226,2722226,2722225,2722222,2110292,2722207,2868153,2802049,2722229,2721144,2108349)
  and c.invalid_reason is null
...
WHERE (C.condition_start_date >= DATEFROMPARTS(2010, 10, 19) and C.condition_start_date <= DATEFROMPARTS(2018, 12, 31))
...
JOIN #Codesets codesets on ((de.device_concept_id = codesets.concept_id and codesets.codeset_id = 6))
) C
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= DATEADD(day,60,P.START_DATE)
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
-- End Correlated Criteria
...
select event_id, person_id, 
  case when DATEADD(day,60,start_date) > start_date then DATEADD(day,60,start_date) else start_date end as end_date
INTO #strategy_ends
